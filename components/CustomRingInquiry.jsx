...
import { hiraDarkBrownText, hiraLightBrownText } from "./constants";
import sendEmail from "../lib/SendEmail";
import SubmitSucess from "./SubmitSuccess";
import CustomOptionPicker from "./CustomOptionPicker";
import CustomRingImageUploader from "./CustomRingImageUploader";
import sendCustomInquiry from "../lib/SendCustomInquiry";
// import sendEmail from "../lib/SendEmail";
// import SubmitSucess from "./SubmitSucess";

const CustomRingInquiry = () => {
  ...
  const [emailRes, setEmailRes] = useState({ status: 200, message: "" });

  const [isAlertVisible, setIsAlertVisible] = useState(false);

  const optionBudget = ["0-$2000", "$2001 - $5000", "$5000+"];
  const optionPreferredContact = ["Email", "Phone"];
  const optionPreferedRingType = [
    "Engagement Ring",
    "Anniversary Ring",
    "Fine Jewelry Ring",
    "Wedding Band",
  ];
  const optionPreferedStone = [
    "Lab Grown",
    "Natural",
    "Colored Gemstones",
    "Not Sure",
  ...
  const [prefRingType, setPrefRingType] = useState(null);
  const [prefStoneType, setPrefStoneType] = useState(null);
  const [prefContact, setPrefContact] = useState(null);
  const [prefBudget, setPrefBudget] = useState(null);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (
        ...
        const req = await sendCustomInquiry(
          firstName,
          lastName,
          email,
          phone,
        ...
        if (req.status === 200) {
          setEmailRes({
            status: req.status,
            message: `Thank you for your message ${firstName}. Our connect team will reach out shortly`,
          });
        }
        resetForm();
      }
  ...
  return (
    <section className="relative z-10 overflow-hidden sm:px-10 bg-white py-4 lg:py-10">
      <div className="container mx-auto mb-3">
        <div className="-mx-4 flex flex-wrap lg:justify-between">
          <div className="w-full px-4 lg:w-1/2 xl:w-6/12">
            <div className="px-3 sm:px-0 mb-12 max-w-[570px] lg:mb-0">
              <h2
                className={`${hiraDarkBrownText} mb-6 text-[32px] font-bold uppercase sm:text-[40px] lg:text-[36px] xl:text-[40px]`}
              >
                Custom Ring Inquiry
              </h2>
              <p className={`text-emerald-900 mb-9 text-base leading-relaxed`}>
              ...
              <div className="mb-8 flex w-full max-w-[370px]">
                ...
                <div className="w-full">
                  <h4 className={`${hiraDarkBrownText} mb-1 text-xl font-bold`}>
                    Our Location
                  </h4>
                  <p className={`${hiraDarkBrownText} text-base`}>
              ...
              <div className="mb-8 flex w-full max-w-[370px]">
                ...
                <div className="w-full">
                  ...
                  <p
                    className={`${hiraDarkBrownText} text-body-color text-base`}
                  >
                    (+1) 647 424 4472
                  </p>
                </div>
          ...
          <div className="w-full px-4 lg:w-1/2 xl:w-5/12">
            <div className="relative rounded-lg bg-white mx-5 p-8 shadow-lg sm:p-12">
              <form>
                ...
                <div>
                  <button
                    type="submit"
                    className="w-full px-6 py-2.5 bg-zinc-600 text-white font-medium text-base leading-tight uppercase rounded shadow-md hover:bg-zinc-700 hover:shadow-lg focus:bg-zinc-900 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-emerald-700 active:text-gray-800 active:shadow-lg transition duration-150ease-in-out"
...
export default CustomRingInquiry;
